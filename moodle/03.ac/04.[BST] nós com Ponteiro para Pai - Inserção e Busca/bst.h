...
struct Node {
    ...
    // Destrutor
    ~Node() { cout << "Node removed: " << this->key << endl; }
};
...
// Constructor 2
BST::BST(int k) { root = new Node(k); }

void BST::bshow() { bshow(root, ""); }
...
void BST::bshow(Node *node, std::string heranca) {
    ...
    if (heranca != "") std::cout << (heranca.back() == 'r' ? "┌───" : "└───");
    if (node == nullptr) {
        std::cout << "#" << std::endl;
        return;
    }
    ...
    while (true) {
        if (k < current->key) {
            if (current->left == nullptr) {
                current->left = new Node(k, nullptr, nullptr, current);
                return;
            }
            current = current->left;
        } else if (k > current->key) {
            if (current->right == nullptr) {
                current->right = new Node(k, nullptr, nullptr, current);
                return;
            }
